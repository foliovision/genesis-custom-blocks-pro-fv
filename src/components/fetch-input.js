...
class FetchInput extends Component {
  ...
  updateSuggestions( value ) {
    ...
    request.then( ( results ) => {
      ...
      //  if results are not empty, show suggestions
      if ( !! results.length ) {
        // update suggestions
        this.setState( {
          results,
  ...
  setInputValidity( isValid ) {
    ...
    if ( ! isValid ) {
      ...
      this.inputRef.current.setCustomValidity( sprintf( __( 'Invalid %s', 'genesis-custom-blocks-pro' ), this.props.field.control ) );
